...
注意：如果没有特殊指明，本文档的所有操作**均在 linker-k8s-01 节点上执行**。
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "rm -rf /etc/kubernetes/cert"
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "rm -rf /etc/kubernetes/encryption-config.yaml"
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "rm -rf /etc/kubernetes/cert/proxy-client*.pem"
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh root@${node_ip} "rm -rf /etc/systemd/system/kube-apiserver.service"

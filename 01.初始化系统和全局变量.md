...
``` bash
hostnamectl set-hostname linker-k8s-01 # 将 linker-k8s-01 替换为当前主机名
```
...
``` bash
cat >> /etc/hosts <<EOF
172.20.10.121 linker-k8s-01
172.20.10.122 linker-k8s-02
172.20.10.123 linker-k8s-03
EOF
```
...
本操作只需要在 linker-k8s-01 节点上进行，设置 root 账户可以无密码登录**所有节点**：

``` bash
ssh-keygen -t rsa 
ssh-copy-id root@linker-k8s-01
ssh-copy-id root@linker-k8s-02
ssh-copy-id root@linker-k8s-03
```
...
输出：
``` text
      Local time: Fri 2021-04-23 15:01:33 CST
  Universal time: Fri 2021-04-23 07:01:33 UTC
        RTC time: Fri 2021-04-23 07:01:33
...
将environment.sh下载,修改后放在 /opt/k8s/bin/下
``` bash 
